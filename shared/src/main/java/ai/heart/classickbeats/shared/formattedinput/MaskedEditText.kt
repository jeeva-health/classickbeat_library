...
import ai.heart.classickbeats.shared.R
import ai.heart.classickbeats.shared.formattedinput.maskers.BaseMasker
import ai.heart.classickbeats.shared.formattedinput.maskers.Masker
import ai.heart.classickbeats.shared.formattedinput.maskers.Masker.Companion.POUND
import ai.heart.classickbeats.shared.formattedinput.masks.CustomMask
import ai.heart.classickbeats.shared.formattedinput.masks.UnselectedMask
...
class MaskedEditText : TextInputEditText {
    ...
    private var maskType: Mask by Delegates.observable(UnselectedMask()) { _, _, newValue ->
        setMasker(newValue)
    }
    ...
    private fun setMasker(mask: Mask) {
        if (mask is UnselectedMask) {
            throw Exception(context.getString(R.string.exception_unselected_mask))
        }
        masker = when (mask) {
    ...
    private fun handleCustomMask(mask: Mask): Masker {
        return when {
            maskPattern.contains(POUND).not() -> {
                throw Exception(context.getString(R.string.exception_mask_pound))
            }
            returnMaskPattern.contains(POUND).not() -> {
                throw Exception(context.getString(R.string.exception_return_pound))
            }
            maskPattern.count { it == POUND } != returnMaskPattern.count { it == POUND } -> {
                throw Exception(context.getString(R.string.exception_pound_count))
            }
            else -> {
