...
class HistoryAdapter constructor(
    ...
    PagingDataAdapter<HistoryItem, RecyclerView.ViewHolder>(TimelineItemDiffCallback()) {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        item?.let {
            when (it) {
                is HistoryItem.LogItem -> (holder as TimelineViewHolder).bind(
                    context,
                    it.timeline,
                    itemClickListener
                )
                is HistoryItem.DateItem -> (holder as DateViewHolder).bind(it.date)
            }
        }
    }
...
class TimelineItemDiffCallback : DiffUtil.ItemCallback<HistoryItem>() {
    override fun areItemsTheSame(oldItem: HistoryItem, newItem: HistoryItem): Boolean =
        if (oldItem is HistoryItem.LogItem && newItem is HistoryItem.LogItem) {
