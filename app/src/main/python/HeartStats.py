...
class HeartStats:
    ...
    def peaks_arr(fs, mt, ms, sign, w = np.array([.5, 1., 1.5, 2.]), time_lower_bound = 0.5):
        ...
        for i in w:  # widths
            ...
            len_filler = np.zeros((len(ms)-len(mov_avg))//2) + np.mean(ms) # used to make mov_avg the same size as sign
        ...
        quality = np.var([ms[x] for x in final_peaks])*(len(checker[checker < 0]) - 2 * len(final_peaks) + 2)/len(checker)

        ## Calculating rate
        time_peaks = np.array(mt)[final_peaks.astype(int)]
        time_RR = np.diff(time_peaks, 1, 0,)*1e3  # time between consecutive R peaks (in milliseconds)
    ...
    def HR_stats(data, fs=30):
        ...
        return "HR: {:.3f}, HRV (sdnn): {:.3f}, AFib: ".format(bpm, sdnn, sqi2, sqi3) +
              s + ", Quality: {:.2e}, SQI1: {:.2f}, SQI2: {:.2f}, SQI3: {:.2f}".format(quality, sqi1, sqi2, sqi3)